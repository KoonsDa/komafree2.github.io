...
function cardAbilityManagerHtml() {
  ...
  const items = cardAbilities().filter((ability) => !ability.deleted).map((ability) => { const targetBadges = cardAbilityTargetSummary(ability).split(" · ").map((target) => `<span>${escapeHtml(target)}</span>`).join(""); const drawText = ability.active ? `가중치 ${ability.weight} · ${formatCardAbilityProbability(ability.weight, activeWeightTotal)}%` : `가중치 ${ability.weight} · 현재 부여 제외`; return `<div class="card-ability-list-row"><div class="card-ability-list-name"><strong>${ability.icon ? `${escapeHtml(ability.icon)} ` : ""}${escapeHtml(ability.name)}</strong></div><span class="card-ability-list-target" data-label="적용 대상">${targetBadges}</span><span class="card-ability-list-rate" data-label="능력 부여 확률">${drawText}</span><span class="card-ability-list-status-wrap" data-label="상태"><span class="card-ability-list-status pill ${ability.active ? "success" : "danger"}">${ability.active ? "ON" : "OFF"}</span></span><div class="card-ability-list-actions"><button class="button secondary compact" data-action="edit-card-ability" data-id="${ability.id}">수정</button><button class="button secondary compact" data-action="toggle-card-ability" data-id="${ability.id}">${ability.active ? "사용 중지" : "사용 재개"}</button><button class="button danger compact" data-action="delete-card-ability" data-id="${ability.id}">삭제</button></div></div>`; }).join("");
  return `<div class="card-ability-manager"><div class="section-heading"><div><h3>특수능력 관리</h3><p class="muted">카드를 획득하거나 업그레이드할 때 어떤 특수능력이 붙을지 결정합니다.</p></div><button class="button success compact" data-action="new-card-ability">+ 특수능력 추가</button></div><div class="card-ability-list"><div class="card-ability-list-head"><span>특수능력</span><span>적용 대상</span><span>능력 부여 확률</span><span>상태</span><span>관리</span></div>${items || '<div class="empty">등록된 특수능력이 없습니다.</div>'}</div><h3 class="ability-effect-heading">하루 최대 보너스</h3></div>`;
}
...
function updateCardAbilityModalProbability(form) {
  ...
  form.querySelector("[data-ability-probability]").textContent = active ? `${formatCardAbilityProbability(weight, total)}%` : "현재 부여 제외";
}
