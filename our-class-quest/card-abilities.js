...
function cardAbilityManagerHtml() {
  ...
  const items = cardAbilities().filter((ability) => !ability.deleted).map((ability) => { const targetText = cardAbilityTargetSummary(ability); const drawText = ability.active ? `가중치 ${ability.weight} · ${formatCardAbilityProbability(ability.weight, activeWeightTotal)}%` : `가중치 ${ability.weight} · 현재 뽑기 제외`; return `<div class="card-ability-list-row"><div class="card-ability-list-name"><strong>${ability.icon ? `${escapeHtml(ability.icon)} ` : ""}${escapeHtml(ability.name)}</strong><span class="pill ${ability.active ? "success" : "danger"}">${ability.active ? "ON" : "OFF"}</span></div><span class="card-ability-list-target">${targetText}</span><span class="card-ability-list-rate">${drawText}</span><span class="card-ability-list-status pill ${ability.active ? "success" : "danger"}">${ability.active ? "ON" : "OFF"}</span><div class="card-ability-list-actions"><button class="button secondary compact" data-action="edit-card-ability" data-id="${ability.id}">수정</button><button class="button secondary compact" data-action="toggle-card-ability" data-id="${ability.id}">${ability.active ? "사용 중지" : "사용 재개"}</button><button class="button danger compact" data-action="delete-card-ability" data-id="${ability.id}">삭제</button></div></div>`; }).join("");
