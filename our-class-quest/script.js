...
function loadData() {
  try {
    ...
    saved.pointUseRequests = (Array.isArray(saved.pointUseRequests) ? saved.pointUseRequests : []).filter((request) => request && /^\d{4}-\d{2}-\d{2}$/.test(String(request.date || "")) && ["pending", "completed", "rejected", "reversed"].includes(request.status)).map((request) => ({ id: String(request.id || crypto.randomUUID()), autoApproved: request.autoApproved === true, itemId: String(request.itemId || ""), itemName: String(request.itemName || ""), studentId: String(request.studentId || ""), date: String(request.date), price: Math.max(0, Number(request.price) || 0), status: request.status, createdAt: String(request.createdAt || new Date().toISOString()), resolvedAt: request.resolvedAt ? String(request.resolvedAt) : null, reversedAt: request.reversedAt ? String(request.reversedAt) : null, reversedBy: String(request.reversedBy || ""), reversalHistoryId: String(request.reversalHistoryId || ""), originalHistoryId: String(request.originalHistoryId || "") }));
...
app.addEventListener("submit", async (event) => {
  ...
  if (form.id === "class-info-form") {
    ...
    data.classSettings = { ...data.classSettings, appName: appName.slice(0, 50), className: className.slice(0, 50), teacherName: teacherName.slice(0, 30), autoApproveAt21: formData.get("autoApproveAt21") !== "off" }; saveData(); render(); toast("학급 정보를 저장했습니다."); saveFirebaseClassSettings(); return;
  }
