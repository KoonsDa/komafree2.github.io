(() => {
  ...
  function cloudShopSignature(items, requests, openTime = pointShopOpenTime) {
    ...
    const requestKeys = ["autoApproved", "id", "status", "studentId", "itemId", "itemName", "date", "price", "approvalRequired", "createdAt", "resolvedAt", "cancelledAt", "cancelledBy"];
  ...
  function openItemRequests(itemId) {
    ...
    const rows = requests.map((request) => { const student = studentById(request.studentId); const studentLabel = student ? `${studentNumber(student)}번 ${student.name}` : "삭제된 학생"; return `<article class="point-request-row"><div><strong>${escapeHtml(studentLabel)}</strong><span>${escapeHtml(item.name)} · ${request.price}P</span><small>${labels[request.status]}${request.autoApproved ? " (자동 승인)" : ""} · ${new Date(request.createdAt).toLocaleTimeString("ko-KR", { hour: "numeric", minute: "2-digit" })}</small></div>${request.status === "pending" ? `<div class="button-row"><button class="button success compact" data-action="approve-point-use" data-id="${request.id}">승인</button><button class="button danger compact" data-action="reject-point-use" data-id="${request.id}">거절</button></div>` : ""}</article>`; }).join("");
